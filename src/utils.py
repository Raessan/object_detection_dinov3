...
def plot_detections(image_np, boxes_xyxy, scores, labels, class_names=None, figsize=(10,10)):
    ...
    for i, box in enumerate(boxes_np):
        ...
        ax.add_patch(rect)
        ax.text(x1, max(0, y1-2), label_text, fontsize=10, color='white',
...
def probs_to_logits(prob_tensor, eps=1e-6):
    """Convert probabilities (in [0,1]) to logits safely (clamp to [eps,1-eps])."""
    p = prob_tensor.clamp(min=eps, max=1.0 - eps)
    return (p / (1.0 - p)).log()

def build_outputs_from_targets(cls_targets, reg_targets, ctr_targets, device=None, eps=1e-6):
    """
    cls_targets: list[level] (B, C, H, W) with 0/1 targets
    reg_targets: list[level] (B, 4, H, W) distances (in pixels) (these are already predictions)
    ctr_targets: list[level] (B, 1, H, W) in [0,1] (centerness)
    Returns outputs-like dict with logits:
      outputs_gt['cls'] -> list of tensors (B,C,H,W) logits
      outputs_gt['reg'] -> list of tensors (B,4,H,W) (use reg_targets as-is)
      outputs_gt['ctr'] -> list of tensors (B,1,H,W) logits (logit(ctr_targets))
    """
    outputs_gt = {'cls': [], 'reg': [], 'ctr': []}
    for lvl in range(len(cls_targets)):
        cls_p = cls_targets[lvl].to(dtype=torch.float32)
        cls_logits = probs_to_logits(cls_p, eps=eps)
        outputs_gt['cls'].append(cls_logits)

        # regression: treat target distances as perfect predictions (they are already in pixels)
        reg_pred = reg_targets[lvl].to(dtype=torch.float32)
        outputs_gt['reg'].append(reg_pred)

        # centerness: convert probability -> logits
        ctr_p = ctr_targets[lvl].to(dtype=torch.float32)
        ctr_logits = probs_to_logits(ctr_p, eps=eps)
        outputs_gt['ctr'].append(ctr_logits)
